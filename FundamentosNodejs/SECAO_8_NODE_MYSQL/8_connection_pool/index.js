...
app.post('/book/insertbook', (req, res) => {
  ...
  const data = ['title', 'pag', 'author', title, pag, author]
...
app.post('/book/updatebook', (req, res) =>{
  ...
  const data = ['title', title,'pag', pag, 'author', author, 'idbooks', id]
...
app.get('/books/:id', (req, res)=> {
  ...
  const id = req.params.id
  const queryId = `SELECT * FROM books WHERE ?? = ?`
  const data = ['idbooks', id]

  pool.query(queryId,data, function (err, data) {
    if(err){
      console.log(err)
      return
    }
